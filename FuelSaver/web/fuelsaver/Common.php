...
//Connection Settings @0-390E7272
...
//Initialize Common Variables @0-BAD6F018
...
$CCSDesign = "";
define("CCS_ENCRYPTION_KEY_FOR_COOKIE", 'fuelsaver');
...
//FuelSaver Connection Class @-872A8753
class clsDBFuelSaver extends DB_Adapter
{
    function clsDBFuelSaver()
    {
        $this->Initialize();
    }
    ...
    function Initialize()
    {
        ...
        $this->SetProvider($CCConnectionSettings["FuelSaver"]);
        parent::Initialize();
...
//End FuelSaver Connection Class
...
function CCSetALCookie($login, $password) {
    ...
    CCSetCookie("FuelSaverLogin", $result, time() + CCS_EXPIRATION_DATE, "/", "", false);
}
//End CCSetALCookie

//CCRefreshALCookie @0-92484D18
function CCRefreshALCookie($expirationDate) {
    if (CCS_SLIDING_EXPIRATION) {
        if (($expirationDate - (CCS_EXPIRATION_DATE / 2)) > time()) {
            list($login, $password, $expDate) = CCParseALCookie("FuelSaverLogin");
...
//CCCheckAutoLoginCookies @0-77754D2A
function CCCheckAutoLoginCookies() {
    if (!CCGetUserLogin()) {
        $parts = CCParseALCookie("FuelSaverLogin");
...
//CCLoginUser @0-80AC5A74
function CCLoginUser($login, $password)
{
    CCLogoutUser();
    $db = new clsDBFuelSaver();
    $SQL = "SELECT Id, UserRole, UserPassword FROM users WHERE UserId=" . $db->ToSQL($login, ccsText) . " AND UserPassword=" . $db->ToSQL($password, ccsText);
    $db->query($SQL);
    $Result = $db->next_record();
    if ($Result) {
        CCSetSession("UserID", $db->f("Id"));
        CCSetSession("UserLogin", $login);
        CCSetSession("GroupID", $db->f("UserRole"));
